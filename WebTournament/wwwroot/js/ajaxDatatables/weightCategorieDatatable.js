let GetWeightCategorieDatatable = function () {
    ...
    function init() {
        const $tbl = $('#item-tbl').DataTable({
            ...
            ajax: function(data, callback, settings) {
                dtAjaxHandler('/WeightCategorie/List', data, callback)
        ...
        $tbl.on('click', 'button[data-action="showEditItemModal"]', function (ev) {
            let $row = $(ev.currentTarget).closest('tr');
            let itemId = $row.attr('id');
            $.get(`/WeightCategorie/${itemId}/EditIndex`, function(data){
