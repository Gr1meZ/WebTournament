    ...
    function init() {
        const $tbl = $('#item-tbl').DataTable({
            ...
            columnDefs: [
                ...
                { targets: 1, title: "Номер", data: "beltNumber" },
                { targets: 2, title: "Сокращенное название", data: "shortName" },
                { targets: 3, title: "Полное название", data: "fullName" },
        ...
        $tbl.on('click', 'button[data-action="showEditItemModal"]', function (ev) {
            let $row = $(ev.currentTarget).closest('tr');
            let itemId = $row.attr('id');
            $.get(`/Belt/${itemId}/EditIndex`, function(data){
